...
// A one-shot badge: either earned or not.
function flag(
  id: string,
  title: string,
  description: string,
...
export function computeNicheAchievements(s: BehavioralSignals): Achievement[] {
  return [
    ...
    flag('vampire-hours', 'Vampire Hours', 'Train between midnight and 4 AM', 'cloudy-night', 'epic', s.trainedMidnightTo4),
    ...
    flag('four-seasons', 'Four Seasons', 'Train in all four seasons of one year', 'leaf', 'epic', s.hasAllFourSeasons),

    // Variety & balance
    flag('well-rounded', 'Well-Rounded', 'Hit push, pull, and legs within one week', 'body', 'rare', s.hasFullPPLWeek),
    progressBadge('renaissance-lifter', 'Renaissance Lifter', 'Log 50 different exercises', 'color-palette', 'epic', s.distinctExercises, 50),
    ...
    // Calendar & holiday (hidden-gem dated badges)
    flag('new-year', 'New Year, New PR', 'Train on January 1st', 'sparkles', 'rare', s.trainedNewYearsDay),
    flag('turkey-burn', 'Turkey Burn', 'Train on Thanksgiving Day', 'restaurant', 'epic', s.trainedThanksgiving),
    flag('gym-on-christmas', 'Gym on Christmas?', 'Train on December 25th', 'gift', 'epic', s.trainedChristmas),
    flag('leap-of-faith', 'Leap of Faith', 'Train on February 29th', 'calendar-number', 'legendary', s.trainedLeapDay),
  ];
}
