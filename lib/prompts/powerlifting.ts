...
export class PowerliftingPromptStrategy implements PromptStrategy {
  async buildPrompt(context: WorkoutContext, analysis: WorkoutAnalysis, customRequest?: string, workoutTypeOverride?: WorkoutSplit, previousWorkout?: GeneratedWorkout): Promise<string> {
    ...
    if (workoutFilters && workoutFilters.excludedWorkoutIds.length > 0) {
      ...
      console.log(`🔍 Powerlifting prompt filter: ${totalBefore} → ${availableWorkouts.length} exercises (filtered out ${workoutFilters.excludedWorkoutIds.length})`);
    }
    console.log('🔍 Available powerlifting workouts:', availableWorkouts.length);
